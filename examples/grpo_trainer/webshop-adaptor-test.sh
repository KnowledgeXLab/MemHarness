...
#SBATCH --mem=500G 
#SBATCH --output=logs/mem_adaptor/webshop/adaptor_local-7b-test_%j.out
#SBATCH --error=logs/mem_adaptor/webshop/adaptor_local-7b-test_%j.err
set -x
set -euo pipefail
...
REASONING_MODEL_PATH='models/save_models/mem_adaptor/webshop/train_adaptor-same-7B-cold_start_20260519_epoch1-with_agentic_memory-retrieve_memory_text-self_distill/global_step_115/actor-hf'
...
if [[ ! -f "${SCRIPT_DIR}/memory_eval_helpers.sh" ]]; then
  ...
  echo "[error] Submit from repo root: sbatch examples/grpo_trainer/webshop-adaptor-test.sh" >&2
  exit 1
fi
...
EXPERIMENT_NAME="train_adaptor-same-7B-cold_start_20260519_epoch1-step-115-adaptor_qwen2.5-7b-test"
