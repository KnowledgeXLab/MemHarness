#!/usr/bin/env bash
# MemAdaptor + AlfWorld：与 train_alfworld-adaptor-local.sh 相同，但 **Reasoning 与 MemAdaptor 共用同一份 HF 权重**（同一路径、同参数量级）。
# 仍使用两套资源池：global_pool 上冻结的 actor+vLLM，mem_adaptor 池上只训 Adaptor；仅权重文件相同，并非合并为单个推理进程。
# 与 train_alfworld-adaptor-remote.sh 的差异仍为 ENGINE=vllm、无 openai_api。
set -x
set -euo pipefail
...
# mem_adaptor 专用池每节点 GPU 数（与 Reasoning 池分离；权重路径相同，仍各自加载一份模型）
...
EXPERIMENT_NAME="actor-same_model_adaptor-1.5b-train"
...
LOG_FILE="${EXP_DIR}/train_alfworld_adaptor_local_same_model-$(date +%Y%m%d_%H%M%S).log"
