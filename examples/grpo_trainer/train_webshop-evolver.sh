#!/usr/bin/env bash
#SBATCH --job-name=e04-web-evolver
...
#SBATCH --mem=1000G 
#SBATCH --output=logs/mem_adaptor/webshop/evolver_%j.out
#SBATCH --error=logs/mem_adaptor/webshop/evolver_%j.err
...
# MODEL_PATH='models/save_models/mem_adaptor/cold_start/webshop/qwen2.5-3b-cold-start-20260519/global_step_250'
...
if [[ ! -f "${SCRIPT_DIR}/memory_eval_helpers.sh" ]]; then
  ...
  echo "[error] Submit from repo root: sbatch examples/grpo_trainer/train_webshop-evolver.sh" >&2
  exit 1
fi
...
EXPERIMENT_NAME="evolver-7B-cold_start_20260706_epoch2"
