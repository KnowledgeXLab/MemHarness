...
from exp_rl.experience import config as exp_config
from exp_rl.experience import prompts as exp_prompts
...
class VectorDBClient:
    ...
    def search_principle(self, question: str, top_k: int) -> List["tuple[str, float, Dict]"]:
        ...
        try:
            ...
            # Convert to the expected format (principle_id, similarity_score, ExperiencePrinciple)
            converted_results = []
            for item in results:
    ...
    def get_vdb_status(self) -> dict:
        ...
        try:
            ...
            # logger.info(f"VDB status: {status['data_status']}")
...
class Trajectory:
    ...
    question: str  # 用户查询
    log: List[Dict[str, Any]]  # 轨迹日志
    final_outcome: bool  # True for success, False for failure
    retrieved_principles: Dict[str, float]  # {principle_id: similarity_score}  # 检索到的原则及其相似度
    golden_answer: str  # 正确答案
...
class RetrievedExperiencePackage:
    ...
    positive_examples: List[Trajectory]  # Compacted successful trajectories, 将会只保留log中observation和thought的前k个字符
    negative_examples: List[Trajectory]  # Compacted failed trajectories, 将会只保留log中observation和thought的前k个字符


# --- 3. The Experience Manager ---
...
class ExperienceManager:
    ...
    def retrieve(self, question: str, top_k: int = exp_config.TOP_K_PRINCIPLES) -> List[RetrievedExperiencePackage]:
        ...
        for p_id, sim_score, item in search_results:     
            ...
            # 检测是否为模型崩塌时加入的principle，如果是则不加入，描述开头是n个连续的!则崩塌了
    ...
    def reflection(self, actor_rollout_wg, export: bool = True) -> None:
        ...
        # 根据轨迹buffer里的轨迹更新所有用到的principle的metric score 
    ...
    def _batch_llm_call_with_api(self, prompts: List[str], max_new_tokens: int, batch_size: int = 32, state="", show_progress: bool = True) -> Optional[List[str]]:
        ...
        # Normalize API endpoint: accept base_url (e.g., http://host:port/v1) and auto-append the
        # standard OpenAI-compatible path when needed.
        if api_url:
