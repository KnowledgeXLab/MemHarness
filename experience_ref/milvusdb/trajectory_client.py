...
class TrajectoryVectorDBClient(BaseVectorDBClient):
    ...
    def _create_collection_if_not_exist(self):
        '''
        检查是否存在 trajectories 集合，如果不存在则创建。
        '''
    ...
    def add_trajectory(self, trajectory_id: str, query: str, log: str, 
        ...
        '''
        向 Milvus 中插入一个新的轨迹记录。
        
        Args:
            trajectory_id: 轨迹唯一标识符
            query: 查询文本
            log: 执行日志
            final_outcome: 最终结果（成功/失败）
            retrieved_principles: 检索到的经验原则ID列表
            golden_answer: 真实答案
        '''
    ...
    def delete_trajectory(self, trajectory_id: Optional[str] = None, before_timestamp: Optional[int] = None):
        """
        删除轨迹记录。可以按 ID 精确删除，也可以根据创建时间批量删除。

        Args:
            trajectory_id (Optional[str]): 轨迹 ID（优先使用）。
            before_timestamp (Optional[int]): 删除该时间戳之前创建的所有轨迹。
    ...
    def update_trajectory(self, trajectory_id: str, **kwargs) -> bool:
        """
        更新trajectory的字段
        可更新字段: query, log, final_outcome, retrieved_principles, merged_trajectory_ids, golden_answer
        """
        backup_data = None
        temp_id = None
