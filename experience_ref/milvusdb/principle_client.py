...
class PrincipleVectorDBClient(BaseVectorDBClient):
    def __init__(self,
                 ...
                 api_url: str = "http://10.140.37.75:8000/v1",
                 api_key: str = "empty",
                 model_name: str = "bge_m3"):
        if experiment_name:
            experiment_name = experiment_name.replace(".", "_")
        # 如果提供了实验名，使用实验名生成集合名；否则使用默认集合名
        if experiment_name:
    ...
    # 仅用于更新路径的轻量清洗，避免 None/NaN/超长引发 milvus-lite 崩溃
    ...
    def add_principle(self, principle_id: str, description: str, structure: List[Dict[str, str]],
        ...
        data = {
            ...
            "updated_at": current_time, # 初始时与created_at相同
        }
    ...
    def search_principles(self, query: str, top_k: int) -> List[Tuple[str, float, Dict]]:
        ...
        Returns:
            List of (principle_id, similarity_score, entity_data)
        '''
    ...
    def delete_principle(self, principle_id: Optional[str] = None, before_timestamp: Optional[int] = None):
        """
        删除经验原则记录。可以按 ID 精确删除，也可以根据创建时间批量删除。

        Args:
            principle_id (Optional[str]): 原则 ID（优先使用）。
            before_timestamp (Optional[int]): 删除该时间戳之前创建的所有原则。
    ...
    def clean_low_metric_principles(self, threshold: float) -> int:
        """按 metric_score 阈值批量删除原则（metric_score < threshold）。"""
    ...
    def update_principle(self, principle_id: str, **kwargs) -> bool:
        """
        更新principle的字段 - 修正版本，只更新schema中存在的字段
        可更新字段: description, structure, principle_type, metric_score, usage_count, 
                success_count, successful_trajectory_ids, failed_trajectory_ids
        """
        backup_data = None
        temp_id = None
