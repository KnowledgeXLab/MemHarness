...
from verl.memory.memory_store import MilvusMemoryStore
from verl.memory.types import MemoryRecord


def _default_memory_dir(memory_config, rollout_log_dir: str | None) -> str:
    memory_dir = memory_config.store_dir
    if memory_dir:
        return memory_dir
    base_dir = rollout_log_dir or os.getcwd()
    return os.path.join(base_dir, "memory_store")
...
def start_local_memory_server(memory_config, task_name: str, rollout_log_dir: str | None):
    ...
    memory_dir = _default_memory_dir(memory_config=memory_config, rollout_log_dir=rollout_log_dir)
    ...
    if collection_name:
        command.extend(["--collection-name", str(collection_name)])

    rebuild_source_path = memory_config.rebuild_source_path
    if rebuild_source_path:
        command.extend(["--rebuild-source-path", str(rebuild_source_path)])

    rebuild_source_collection_name = memory_config.rebuild_source_collection_name
    if rebuild_source_collection_name:
        command.extend(["--rebuild-source-collection-name", str(rebuild_source_collection_name)])

    embedding_api_url = memory_config.embedding_api_url
    if embedding_api_url:
        command.extend(["--embedding-api-url", str(embedding_api_url)])

    embedding_api_key = memory_config.embedding_api_key
    if embedding_api_key:
        command.extend(["--embedding-api-key", str(embedding_api_key)])

    embedding_model = memory_config.embedding_model
    if embedding_model:
        command.extend(["--embedding-model", str(embedding_model)])

    if memory_config.only_successful:
        command.append("--only-successful")
    if memory_config.clean_before_init:
        command.append("--clean-before-init")

    rebuild_insert_batch_size = memory_config.rebuild_insert_batch_size
    if rebuild_insert_batch_size is not None:
        command.extend(["--rebuild-insert-batch-size", str(rebuild_insert_batch_size)])

    rebuild_embedding_batch_size = memory_config.rebuild_embedding_batch_size
    if rebuild_embedding_batch_size is not None:
        command.extend(["--rebuild-embedding-batch-size", str(rebuild_embedding_batch_size)])
    ...
    while time.time() < deadline:
        if process.poll() is not None:
            raise RuntimeError(
                f"Local memory server exited early with code {process.returncode}. "
    ...
    try:
        ...
        process.wait(timeout=5)
    raise RuntimeError(
        f"Timed out waiting for local memory server at {health_url}. "
        f"Last error: {last_error}. Log: {log_path}"
    )


class _MemoryServiceHandler(BaseHTTPRequestHandler):
    server_version = "VerlMemoryServer/1.0"
