...
def _normalize_collection_name(
    ...
    if use_timestamp:
        ...
        return f"agent_memories_{normalized}_{timestamp}".strip("_")
...
class RemoteHTTPMemoryStore(BaseMemoryStore):
    ...
    def update_records(self, updates: Iterable[dict]) -> int:
        ...
        url = f"{self.base_url}/memories/update"
        payload = {"updates": buffered_updates}
        try:
            response = requests.post(url, json=payload, timeout=self.timeout)
            response.raise_for_status()
            data = response.json()
            ...
            print(f"Failed to update memory records in VDB: {exc}")
        except (TypeError, ValueError) as exc:
            print(f"Failed to parse memory update response: {exc}")
        return 0
...
class MilvusMemoryStore(BaseMemoryStore):
    def __init__(
        ...
        self.db_path = os.path.join(store_dir, "milvus_memory.db")
    ...
    def initialize(self, mode: str, clean_before_init: bool = False) -> None:
        ...
        if clean_before_init and os.path.isdir(self.store_dir):
            ...
            self.client = MilvusClient(self.db_path)
            self.logger = self._build_logger()
            self.logger.info("Cleaned existing db files before initialize store_dir=%s", self.store_dir)
        ...
        if should_rebuild:
            rebuilt = self.rebuild_from_path(
                source_path=self.rebuild_source_path,
                source_collection_name=self.rebuild_source_collection_name,
            )
            self.logger.info(
                "Initialized by rebuilding %s records from source=%s into collection=%s",
                rebuilt,
    ...
    def _entities_from_payload_rows(self, rows: list[dict], progress_label: str | None = None) -> list[dict]:
        missing_vector_rows = [row for row in rows if self._vector_field_name not in row]
        ...
        for row in rows:
            row_copy = dict(row)
            if self._vector_field_name not in row_copy:
                row_copy[self._vector_field_name] = embedded_vectors[embedded_index]
                embedded_index += 1
