...
def main(config):
    ...
    if config.rollout.memory.enabled:
        from verl.memory.local_service import start_local_memory_server

        memory_server_handle = start_local_memory_server(
            memory_config=config.rollout.memory,
            task_name=config.agentgym.task_name,
            rollout_log_dir=config.rollout.rollout_log_dir,
        )
        if memory_server_handle is not None:
            OmegaConf.set_struct(config.rollout.memory, True)
            with open_dict(config.rollout.memory):
                config.rollout.memory.backend = "http"
                config.rollout.memory.vdb_base_url = memory_server_handle.base_url
    ...
    try:
        ...
        ordered_categories = [category_file.split(".")[0] for category_file in category_files]
        if "unknown" in category_success_bucket:
            ordered_categories.append("unknown")

        for category in ordered_categories:
            category_scores = np.array(category_success_bucket[category])
            if len(category_scores) == 0:
                continue
            print(f"Category: {category}")
            print(f"Avg@{config.data.n_samples}: {np.mean(category_scores)}")
            print(f"Pass@{config.data.n_samples}: {np.mean(np.max(category_scores, axis=-1) > 0)}")
    finally:
        if memory_server_handle is not None:
            memory_server_handle.close()
