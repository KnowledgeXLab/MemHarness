...
def parse_thought_action(text: str) -> Tuple[Optional[str], Optional[str]]:
    """Parse legacy AlfWorld string ``Thought: ... Action: ...``."""
...
_TASK_LINE_RE = re.compile(r"Your task is to:\s*([^\n]+)", re.I)
_AVAILABLE_ACTIONS_RE = re.compile(r"AVAILABLE ACTIONS:\s*(.+)$", re.I | re.DOTALL)

# Mirrors env.memory.append_retrieval_hint / ppo_trainer.yaml env.memory.retrieval_instruction_prompt
...
def _fmt_admissible_memadaptor(actions: Sequence[str]) -> str:
    # Matches agent_system/environments/env_manager.build_text_obs reformatted_admissible_actions
    return "\n ".join(f"'{s}'" for s in actions if s != "help")


def split_observation_and_admissibles(raw_human: str) -> Tuple[str, List[str]]:
    """Split AgentTraj-L observation text into env observation + admissible command list."""
    m = _AVAILABLE_ACTIONS_RE.search(raw_human)
    if not m:
        return raw_human.strip(), []
    actions_blob = m.group(1).strip()
    obs = raw_human[: m.start()].strip()
    parts = [p.strip() for p in actions_blob.split(",") if p.strip()]
    return obs, parts


def extract_task_description(raw_human: str) -> str:
    m = _TASK_LINE_RE.search(raw_human)
    return m.group(1).strip() if m else ""


_TASK_LINE_START_RE = re.compile(r"^\s*your task is to:", re.I)


def observation_text_for_history(raw_human: str) -> str:
    """Text stored in Step-k history lines: no AVAILABLE ACTIONS blob, no duplicate task line (template already has task_description)."""
    obs_core, _ = split_observation_and_admissibles(raw_human)
    lines = [ln for ln in obs_core.splitlines() if not _TASK_LINE_START_RE.match(ln)]
    return "\n".join(lines).strip()


def _mem_history_block(completed: List[Tuple[str, str]], history_length: int) -> Tuple[str, int]:
    """Match agent_system/memory/memory.py TrajectoryMemory.fetch formatting."""
    ...
    for j, (obs_text, act) in enumerate(recent):
        ...
        lines.append(f"Step {step_num}:{act} {obs_text}\n")
    return "\n".join(lines).strip(), valid_len


def build_memadaptor_user_message(
    *,
    obs_core: str,
    ...
    adm_s = _fmt_admissible_memadaptor(admissibles)
    if len(completed_history) == 0:
        body = ALFWORLD_TEMPLATE_NO_HIS.format(
            current_observation=obs_core,
            admissible_actions=adm_s,
        )
    else:
        ah, valid_len = _mem_history_block(completed_history, history_length)
        td = task_description if task_description.strip() else "(unknown task)"
        body = ALFWORLD_TEMPLATE.format(
            task_description=td,
...
def validate_memory_record(rec: Dict[str, Any], *, path_hint: str, line_no: int) -> None:
    """Ensure JSONL lines contain fields needed for grouping, sorting, and injection."""
...
def iter_memory_jsonl(path: str) -> Iterable[Dict[str, Any]]:
    """Yield dict lines from JSONL, validating required teacher-memory fields per line."""
    with open(path, "r", encoding="utf-8") as f:
...
def resolve_memory_group_value(rec: Dict[str, Any], group_key: str) -> str:
    """Join key matching alfworld_train ``episode['item_id']``.

    For ``dataset_item_id``, only ``metadata["dataset_item_id"]`` is used (``extract_memory_records.py``).
    """
...
class OpenAICompatClient:
    """OpenAI-compatible chat API via httpx + OpenAI SDK (same stack as extract_memory_records.build_client)."""
...
def build_messages_for_episode(
    ...
    *,
    ...
    # Teacher memory JSONL source_step aligns with emitted assistant turns (excluding skipped OK.)
    memory_turn_idx = 0
    gpt_ord = 0
    task_hint = ""
    task_description = ""
    completed_history: List[Tuple[str, str]] = []
    last_admissibles: Optional[List[str]] = None
    ...
    for turn in conv:
        ...
        if role == "human":
            if memadaptor_user_template and _is_agenttraj_instruction_banner(val):
                continue
            last_human_raw = val
            low = val.lower()
            if "your task is to:" in low or "your task is to" in low:
                task_hint = val
            td = extract_task_description(val)
            if td:
                task_description = td

            if memadaptor_user_template:
                obs_core, adm = split_observation_and_admissibles(val)
                if not adm and last_admissibles is not None:
                    adm = list(last_admissibles)
                if adm:
                    last_admissibles = adm
                user_body = build_memadaptor_user_message(
                    obs_core=obs_core,
                    admissibles=adm,
                    task_description=task_description,
                    completed_history=completed_history,
                    history_length=max(0, int(alfworld_history_length)),
        ...
        if role == "gpt":
            ...
            if memory_group is not None and memory_steps is not None and memory_turn_idx in memory_steps:
                ...
                if rec is not None:
                    ...
                    # Retrieval queries should condition on the trajectory observation for this step, not teacher state_text.
                    real_obs = observation_text_for_history(last_human_raw)
                    query_text: Optional[str] = None
                    if llm is not None:
                        sys_prompt = (
                            "You produce exactly ONE plain-text retrieval query for an embodied agent memory bank. "
                        ...
                        user_prompt = (
                            ...
                            "Focus on the immediate decision bottleneck, not generic object locations.\n\n"
                    ...
                    if lab:
                        inner_lines.append(lab.replace("{index}", "1") + mem_txt)
                    else:
                        inner_lines.append(mem_txt)
                    inner = "\n\n".join(inner_lines)
                    memory_user_inj = f"{memory_open}\n{inner}\n{memory_close}"

            messages.append({"role": "assistant", "content": assistant_xml + retrieve_suffix})
            if memory_user_inj is not None:
                messages.append({"role": "user", "content": memory_user_inj})
            stats["turns"] += 1
            if last_human_raw is None:
                raise ValueError(f"missing preceding human turn before gpt turn in episode {item_id}")
            act_plain = action.strip() if action else ""
            completed_history.append((observation_text_for_history(last_human_raw), act_plain))
            last_human_raw = None
            continue
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    p.add_argument(
        ...
        help=(
            "Field used to join JSONL rows to alfworld_train episode item_id (default: dataset_item_id). "
            "dataset_item_id must appear under metadata.dataset_item_id (extract_memory_records.py)."
        ),
    )
    p.add_argument(
        "--memory-match-rate",
        type=float,
        default=0.5,
        help="Fraction of eligible assistant turns that get retrieve+inject (0..1). Default 0.5.",
    )
    ...
    p.add_argument(
        "--write-jsonl",
        default=True,
        help="Also write JSONL next to parquet: train.jsonl / val.jsonl, or all.jsonl with --output.",
    )
    ...
    p.add_argument(
        ...
        help="Inserted inside <memory> before teacher memory_text (align with env.memory.prompt_header).",
    )
    p.add_argument(
        "--adapted-label-template",
        default="Adapted memory principle {index}:\n",
        help="Same semantics as mem_adaptor.adapted_principle_label_template; {index} -> 1.",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        action="store_true",
        help="Keep AgentTraj-L human strings verbatim (disable MemAdaptor ALFWORLD_TEMPLATE prompts).",
    )
    p.add_argument(
        "--alfworld-history-length",
        type=int,
        default=2,
        help="Matches trainer.env.history_length when formatting ALFWORLD_TEMPLATE (except first observation).",
    )
    p.add_argument(
        "--retrieval-instruction-prompt",
        default="",
        help="Like env.memory.append_retrieval_hint; uses {open_tag}/{close_tag}. Empty = default from ppo_trainer.yaml.",
    )
    ...
    p.add_argument(
        ...
        default=8,
        help=(
            "Parallelism over episodes when generating retrieve queries (HTTP). "
            "1 = serial order (same RNG as before). >1 uses one RNG per episode index "
            "(memory step sampling differs from workers=1)."
        ),
    )
    ...
    if url_set ^ model_set:
        raise SystemExit(
            "Either both --retrieve-api-url and --retrieve-model must be set, or neither "
            "(use heuristic retrieve queries)."
        )
    ...
    def run_one_episode(ep_i: int, episode: Dict[str, Any], *, parallel_rng: bool) -> Tuple[Optional[Dict[str, Any]], Dict[str, int]]:
        ...
        try:
            msgs, st = build_messages_for_episode(
                ...
                alfworld_history_length=int(args.alfworld_history_length),
    ...
    def _write_jsonl_from_df(path: str, frame: "pd.DataFrame") -> None:
        """One JSON object per line; same columns as parquet."""
